"""save_all 查找roomid_uid_followers开头的toml文件
...
from bili_global import DIRECTORY_ROOMID_UID_FOLLOWERS, DIRECTORY_ROOMID_FOLLOWERS_GUARD
...
async def save_one(file_url, chuncks_roomid):
    ...
    for i, piece in enumerate(chuncks_roomid):
        ...
        tasklist = []
        for _, uid, _ in piece:
            task = asyncio.ensure_future(webhub.fetch_guard_num(uid=uid))
            tasklist.append(task)
        if tasklist:
            results = await asyncio.gather(*tasklist)
            for roomid_uid_followers, guard_num in zip(piece, results):
                roomid, _, followers = roomid_uid_followers
                list_rooms.append((roomid, followers, guard_num))
    ...
    with open(
            f'{DIRECTORY_ROOMID_FOLLOWERS_GUARD}/roomid_followers_guard{room_range}({orig_len})_{len(list_rooms)}.toml',
            'w', encoding="utf-8") as f:
        toml.dump(dict_title, f)


async def save_all():
    files = [f'{DIRECTORY_ROOMID_UID_FOLLOWERS}/{f}' for f in os.listdir(DIRECTORY_ROOMID_UID_FOLLOWERS)
             if os.path.isfile(f'{DIRECTORY_ROOMID_UID_FOLLOWERS}/{f}')]
    file_urls = []
    for f in files:
